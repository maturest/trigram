...
namespace Maturest\Trigram\Services;
...
class Calendar
{
     ...
     */
    public function lunar2solar($y, $m, $d, $isLeapMonth = false)
    {//参数区间1900.1.31~2100.12.1
        ...
        //$stmap = gmmktime(0, 0, 0, 1, 30, 1900);转换成php后和js的结果不一致
        $stmap = -2203804800;
        $time = ($offset + $d - 31) * 86400 + $stmap;
        $cY = date('Y', $time);
        $cM = date('m', $time);
        $cD = date('d', $time);
     ...
     */
    public function leapMonth($y)//闰字编码 \u95f0
    {
     ...
     */
    public function solar2lunar($y, $m, $d)//参数区间1900.1.31~2100.12.31
    {
        //年份限定、上限
        if ($y < 1900 || $y > 2100) {
        ...
        $m = date('m', $time);
        $d = date('d', $time);
        $offset = (gmmktime(0, 0, 0, $m, $d, $y) - gmmktime(0, 0, 0, 1, 31, 1900)) / 86400;
        ...
        if (date('Y', $todayTime) == $y && idate('m', $todayTime) == $m && date('d', $todayTime) == $d) {
            $isToday = true;
        }
        //星期几
        $nWeek = date('w', $time);
        ...
        $dayCyclical = gmmktime(0, 0, 0, $m, 1, $y) / 86400 + 25567 + 10;
        ...
        $resunt['lMonth'] = $month;
        $resunt['lDay'] = $day;
        $resunt['Animal'] = $this->getAnimal($year);
        $resunt['IMonthCn'] = ($isLeap ? '闰' : '') . $this->toChinaMonth($month);
        $resunt['IDayCn'] = $this->toChinaDay($day);
        $resunt['cYear'] = intval($y);
        $resunt['cMonth'] = intval($m);
        $resunt['cDay'] = intval($d);
        $resunt['gzYear'] = $gzY;
        $resunt['gzMonth'] = $gzM;
        $resunt['gzDay'] = $gzD;
        $resunt['isToday'] = $isToday;
        $resunt['isLeap'] = $isLeap;
        $resunt['nWeek'] = $nWeek;
        $resunt['ncWeek'] = '星期' . $cWeek;
        $resunt['isTerm'] = $isTerm;
        $resunt['Term'] = $Term;
        $resunt['astro'] = $astro;
    ...
    public function getTerm($y, $n)
    {
        ...
        $_info = str_split($_table, 5);
        array_walk($_info, function (&$val) {
            $val = strval(hexdec($val));
        });
...
//        return $s.substr($cMonth*2 - ($cDay < $arr[$cMonth-1] ? 2 : 0),2) + "\u5ea7";//座
        if ($cMonth < 1 || $cMonth > 12)
            return "";
        $s = ['水瓶', '双鱼', '白羊', '金牛', '双子', '巨蟹', '狮子', '处女', '天秤', '天蝎', '射手', '魔羯'];
        $arr = [20, 19, 21, 20, 21, 22, 23, 23, 23, 24, 23, 22];
        if ($cDay < $arr[$cMonth - 1]) {
            $prevIndex = (($cMonth - 2) < 0 ? 11 : $cMonth - 2);
            return $s[$prevIndex];
        }
        return $s[$cMonth - 1];
    }
     ...
     */
    public function toChinaMonth($m)// 月 => \u6708
    {
        if ($m > 12 || $m < 1) {
            return -1;
        } //若参数错误 返回-1
     ...
     */
    public function toChinaDay($d)//日 => \u65e5
    {
        switch ($d) {
            case 10:
                $s = '初十';
